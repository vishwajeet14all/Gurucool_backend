...
class FlowDistributionAlgorithm {
  ...
  async distributeUser(user) {
    ...
    let astrologers = await Promise.all(
      astrologerKeys.map((key) => redisClient.hGetAll(key))
    );
    ...
    const topAstrologers = astrologers.filter(
      (a) => a.isTopAstrologer === "true" && a.toggleAstrologer === "true"
    );
    const regularAstrologers = astrologers.filter(
      (a) => !(a.isTopAstrologer === "true" && a.toggleAstrologer === "true")
    );
    const selectedAstrologer =
      topAstrologers.length && this.totalConnections % 2 === 0
        ? topAstrologers[0]
    ...
    await redisClient.set(
      this.totalConnectionsKey,
      this.totalConnections.toString()
    );
